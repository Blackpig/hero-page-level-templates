...
class Admincp extends Admincp_Controller {
	...
	function create_post ($type_id = FALSE) {
		...
		if ($type['is_standard'] == TRUE) {
			...
			$template_files = array(''=>'Use  Content Type Template') + template_files();
			$title->dropdown('Output Template', 'content_page_template', $template_files, '', FALSE, FALSE, 'This template in your theme directory will be used to display content for this page.  It will override the template specified for the content type - leave blank to use the content-type template.');
	...
	function edit ($id) {
		...
		if ($type['is_standard'] == TRUE) {
			...
			$this->load->helper('template_files');
			$template_files = array(''=>'Use  Content Type Template') + template_files();
			$title->dropdown('Output Template', 'content_page_template', $template_files, $content['page_template'], FALSE, FALSE, 'This template in your theme directory will be used to display content for this page.  It will override the template specified for the content type - leave blank to use the content-type template.');
	...
	function post ($action = 'new', $id = FALSE) {
		...
		if ($action == 'new') {
			$content_id = $this->content_model->new_content(
													...
													$date,
													$custom_fields,
													$this->input->post('content_page_template')
												);
